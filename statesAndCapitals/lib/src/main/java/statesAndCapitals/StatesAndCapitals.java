...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(b -> b.getStateBird().equals("cardinal")).findFirst().orElseThrow();
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(e -> e.getLowestElevationInFeet() < 0);
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(e -> e.getHighestElevationInFeet() > 21000);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(a -> a.getStateAnthem() != null);
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(w -> w.getStateMotto().split(" ").length ==1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(Collectors.averagingDouble(avg -> avg.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(sum -> sum.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(t->t.getTimeZones().toString(),counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(Collectors.groupingBy(t->t.getCapital().getTimeZone(),counting()));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).distinct().filter(b->!b.equals("mockingbird")).collect(toList());
        ...
        Integer maxStateElevation = states.stream().map(StateInfo::getHighestElevationInFeet).max(Integer::compareTo).orElseThrow();
        ...
        StateInfo stateWithLeastDistanceBetweenHighAndLowPoints = states.stream().min(Comparator.comparing(s -> Math.abs(s.getHighestElevationInFeet()-s.getLowestElevationInFeet()))).orElseThrow();
